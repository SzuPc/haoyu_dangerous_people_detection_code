        ...
        if multimask_output and self.samurai_mode:
            if self.kf_mean is None and self.kf_covariance is None or self.stable_frames == 0:
                ...
                if len(non_zero_indices) == 0:
                    ...
                    y_max, x_max = non_zero_indices.max(dim=0).values
                    high_res_bbox = [x_min.item(), y_min.item(), x_max.item(), y_max.item()]
                if torch.any(ious[0][best_iou_inds] > self.stable_ious_threshold):
                ...
                if torch.max(ious[0][best_iou_inds]) < self.stable_ious_threshold:

                    self.stable_frames = 0
                else:
                    if isinstance(best_iou_inds, torch.Tensor):
                        best_idx = best_iou_inds.tolist()
                    else:
                        best_idx = [best_iou_inds]

                    # 遍历每个 batch 中的 best index
                    updated_bbox = []
                    for b, idx in enumerate(best_idx):
                        updated_bbox.append(self.kf.xyxy_to_xyah(high_res_multibboxes[idx]))

                    # 此处根据你的 KF 逻辑，是否 batch 支持，选择取第一个 or 平均（如 batch size = 1 通常直接用第一个）
                    self.kf_mean, self.kf_covariance = self.kf.update(
                        self.kf_mean, self.kf_covariance, updated_bbox[0]
)
            ...
            if self.samurai_mode:
                ...
                if frame_idx > 1:  # Ensure we have previous frames to evaluate
                    for i in range(frame_idx - 1, 1, -1):  # Iterate backwards through previous frames
                        ...
                        if torch.max(iou_score) > self.memory_bank_iou_threshold and \
                            torch.max(obj_score) > self.memory_bank_obj_score_threshold and \
                            (kf_score is None or torch.max(kf_score) > self.memory_bank_kf_score_threshold):
